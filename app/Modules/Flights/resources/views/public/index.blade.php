<div class="container mx-auto px-4 py-10">
    <header class="mb-6">
        <h1 class="text-3xl font-semibold">Search flights</h1>
        <p class="text-zinc-500 mt-1">Powered by {{ ucfirst(config('hk-modules.modules.flights.provider', 'stub')) }} provider.</p>
    </header>

    <x-ui.card class="mb-6">
        <form wire:submit="search" class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
            <div class="md:col-span-1"><x-ui.input wire:model="origin" label="From (IATA)" maxlength="3" required /></div>
            <div class="md:col-span-1"><x-ui.input wire:model="destination" label="To (IATA)" maxlength="3" required /></div>
            ...
            <div class="md:col-span-1">
                <label class="block text-sm font-medium mb-1.5">Cabin</label>
                <select wire:model="cabin" class="w-full rounded-md border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm">
                    <option value="economy">Economy</option><option value="premium_economy">Premium</option>
        ...
        <div class="space-y-3">
            @forelse ($results as $r)
                <div class="rounded-xl border border-zinc-200 dark:border-zinc-800 p-5 flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="text-xs text-zinc-500 uppercase">{{ $r->airline }} · {{ $r->flightNumber }}</p>
                        <p class="font-semibold mt-1 text-lg">{{ $r->origin }} → {{ $r->destination }}</p>
                        <p class="text-sm text-zinc-500">{{ \Illuminate\Support\Carbon::parse($r->departTime)->format('D, d M H:i') }} – {{ \Illuminate\Support\Carbon::parse($r->arriveTime)->format('H:i') }} · {{ floor($r->durationMinutes / 60) }}h {{ $r->durationMinutes % 60 }}m · {{ $r->stops === 0 ? 'Direct' : $r->stops.' stop(s)' }}</p>
                    </div>
                    <div class="text-right">
                        <p class="text-2xl font-semibold">{{ $r->currency }} {{ number_format($r->price, 0) }}</p>
                        <p class="text-xs text-zinc-500 capitalize">{{ str_replace('_', ' ', $r->cabin) }}</p>
                    </div>
                </div>
            @empty
                <x-ui.alert variant="info">No flights found for this route. Try different dates or airports.</x-ui.alert>
            @endforelse
        </div>
    @endif
</div>
