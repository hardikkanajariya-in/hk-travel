...
<body class="min-h-full bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 antialiased">
    <header class="border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur sticky top-0 z-30">
        <div class="mx-auto max-w-7xl px-6 h-16 flex items-center justify-between">
            <a href="{{ $urls->route('home') }}" wire:navigate class="flex items-center gap-2 font-bold text-lg">
            ...
            <nav class="hidden md:flex items-center gap-6 text-sm font-medium">
                ...
                @if ($manager->enabled('tours'))
                    <a href="{{ $urls->route('tours.index') }}" wire:navigate class="hover:text-hk-primary-600">Tours</a>
                @endif
                @if ($manager->enabled('hotels'))
                    <a href="{{ $urls->route('hotels.index') }}" wire:navigate class="hover:text-hk-primary-600">Hotels</a>
                @endif
                @if ($manager->enabled('flights'))
                    <a href="{{ $urls->route('flights.search') }}" wire:navigate class="hover:text-hk-primary-600">Flights</a>
                @endif
                @if ($manager->enabled('activities'))
                    <a href="{{ $urls->route('activities.index') }}" wire:navigate class="hover:text-hk-primary-600">Activities</a>
                @endif
                @if ($manager->enabled('cruises'))
                    <a href="{{ $urls->route('cruises.index') }}" wire:navigate class="hover:text-hk-primary-600">Cruises</a>
                @endif
                @if ($manager->enabled('cars'))
                    <a href="{{ $urls->route('cars.index') }}" wire:navigate class="hover:text-hk-primary-600">Car Rentals</a>
                @endif
                @if ($manager->enabled('visa'))
                    <a href="{{ $urls->route('visa.index') }}" wire:navigate class="hover:text-hk-primary-600">Visa</a>
                @endif
                @if ($manager->enabled('destinations'))
                    <a href="{{ $urls->route('destinations.index') }}" wire:navigate class="hover:text-hk-primary-600">Destinations</a>
                @endif
            </nav>
